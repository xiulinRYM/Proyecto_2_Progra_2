...
void Simulation::runSimulation(GameUI& ui) {
    ...
    while (isRunning && currentTurn <= maxTurns) {
        if (astronaut->getEnergy() == 0) {
            astronaut->applyOxygenDrain(5);
            ui.showMessage("WARNING: No energy! Oxygen draining...");
        }
...
void Simulation::triggerRandomEvent(GameUI& ui) {
    ...
    ui.showMessage("==================");
    ui.showMessage("Repair module manually? Costs 5 HP and 10 Energy. (1=Yes, 0=No)");

  int rep = ui.getPlayerDecision();

    if (rep == 1 && astronaut->getCurrentModule() != nullptr) {
        if (astronaut->getEnergy() < 10) {
            ...
            ui.showMessage("Module repaired. You lost 5 HP and 10 Energy.");
    ...
    if (astronaut->getCurrentModule() != nullptr) {
        ...
        while (i < mod->getActiveEventCount()) {
            ...
            ui.showMessage("Repair? Costs 5 HP and 10 Energy. (1=Yes, 2=Skip all, 0=Skip)");
            ...
            if (r == 1) {
                if (astronaut->getEnergy() < 10) {
                    ...
                    astronaut->applyDamage(5);
                    astronaut->applyEnergyDrain(10);
                    ui.showMessage("Event resolved. You lost 5 HP and 10 Energy.");
